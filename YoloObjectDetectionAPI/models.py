...
class ImageUpload(models.Model):
    ...
    detected_result = models.FloatField(null=True, blank=True)    # 这个字段表示完整的检测结果 可以在数据库中存储空值
    model_name = models.CharField(max_length=255, null=True, blank=True)   #  这个字段表示检测模型的权重名称 可以在数据库中存储空值


# 这里记录了每一个数字和nums整体的检测结果 位置 置信度
